...
@section('title', 'Semua Produk - Sahabat Alvaro Store')

@section('content')
<div class="bg-gray-50 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
            <h1 class="text-3xl font-extrabold text-gray-900 sm:text-4xl">Koleksi Produk Kami</h1>
            <p class="mt-4 text-lg text-gray-500">Temukan berbagai produk farmasi terbaik dan terpercaya.</p>
        </div>

        <!-- Category Filter Buttons -->
        <div class="flex flex-wrap justify-center gap-3 mb-12">
            <a href="{{ route('product.index') }}" 
               class="px-5 py-2.5 rounded-full text-sm font-semibold transition {{ request()->routeIs('product.index') && !request('category') ? 'bg-primary-blue text-white shadow-md' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200' }}">
                Semua Kategori
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('product.category', $cat->slug) }}" 
               class="px-5 py-2.5 rounded-full text-sm font-semibold bg-white text-gray-700 hover:bg-gray-100 border border-gray-200 transition">
                {{ $cat->name }}
            </a>
            @endforeach
        </div>
        ...
        <div class="text-center py-20 bg-white rounded-lg shadow-sm">
            ...
            <p class="mt-1 text-sm text-gray-500">Produk sedang kosong, silakan cek kembali nanti.</p>
        </div>
        @endif
    </div>
</div>
